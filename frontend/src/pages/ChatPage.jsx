 ...
 *
 * New prop:
 *   onMessagesChange(messages) – called whenever the messages array changes,
 *   so the parent layout can pass `messages` down to Navbar for Export.
 */
export default function ChatPage({ sessionId, onSessionChange, onMessagesChange }) {
  ...
  // Notify parent whenever messages change (so Navbar can read them)
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      const response = await api.post(
        "/chat/chat",
        { session_id: sessionId, message: input, stream: false },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      updateMessages([...withUser, assistantMessage]);
            ...
            {messages.length === 0 ? (
              <div className="text-center py-8 sm:py-12 px-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 max-w-5xl mx-auto">
                  <ExampleQuery icon="🩺" text="Crohn's disease treatment options"
                    onClick={() => setInput("What are the latest treatment options for Crohn's disease?")} />
                  <ExampleQuery icon="💊" text="Rheumatoid arthritis therapy"
                    onClick={() => setInput("What are the current therapies for rheumatoid arthritis?")} />
                  <ExampleQuery icon="🧬" text="Type 2 diabetes management"
                    onClick={() => setInput("What are the best practices for managing type 2 diabetes?")} />
                  <ExampleQuery icon="🫀" text="Chronic kidney disease treatment"
                    onClick={() => setInput("What are the treatment approaches for chronic kidney disease?")} />
                  <ExampleQuery icon="🧠" text="Major depressive disorder therapy"
                    onClick={() => setInput("What are the therapeutic options for major depressive disorder?")} />
                </div>
              </div>
            ) : (
