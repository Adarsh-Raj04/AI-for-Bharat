...
async def create_session(
    ...
    session = ChatSession(
        user_id=user.id, session_name=request.session_name or "New Chat"
    )
    db.add(session)
    db.commit()
    db.refresh(session)
        ...
        "messages": [
            {
                ...
                "intent": msg.intent,
                "timeline": msg.timeline or [],  # ← add this
...
async def rename_session(
    session_id: str,
    request: SessionRenameRequest,  # {"session_name": "New Name"}
    current_user: Dict = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    """Rename a session"""
    ...
    new_name = request.session_name.strip()
    if not new_name:
