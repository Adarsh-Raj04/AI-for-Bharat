...
class CacheService:
    ...
    def _connect(self):
        ...
        try:
            ...
            logger.warning(
                f"⚠️ Redis authentication failed: {e}. Check REDIS_URL credentials. Caching disabled."
            )
            self.redis_client = None
        except redis.ConnectionError as e:
            logger.warning(
                f"⚠️ Redis connection failed: {e}. Caching disabled (optional)."
            )
            self.redis_client = None
    ...
    def get_redis_client(self):
        """Expose raw redis client safely"""
...
def get_redis_client():
    ...
    return cache_service.get_redis_client()
